...
export default function CategoryManager({ categories, onCategoryDelete }: CategoryManagerProps) {
    ...
    async function deleteCategory(category: categoryApi.Category, disclosureClose: () => void) {
        ...
        if (!error) {
            ...
            if (error.code === '23503') {
                console.log('TODO: add client side error handling', "This category cannot be deleted because it has products", error);
    ...
    function renderCustomCategory(category: categoryApi.Category) {
        return (
            <Disclosure>
                {({ open, close }) => (
                    <li className={classNames("flex items-center justify-between gap-x-2", open ? 'py-2' : 'py-3.5')} key={category.id}>
                        <p className="text-sm font-semibold leading-6 text-gray-900 truncate">{ category.name }</p>
                        {
                        ...
                        <Disclosure.Panel className="border-l-4 border-yellow-400 bg-yellow-50 p-2 flex flex-shrink-0">
                            <div className="ml-2">
                                <p className="text-sm text-yellow-700">
                                    ...
                                    <button onClick={() => deleteCategory(category, close)} className="font-medium text-yellow-700 underline hover:text-yellow-600">Yes</button>
                                    {' | '}
                                    <Disclosure.Button className="font-medium text-yellow-700 underline hover:text-yellow-600">No</Disclosure.Button>
                                </p>
                            </div>
                        </Disclosure.Panel>
                    </li>
