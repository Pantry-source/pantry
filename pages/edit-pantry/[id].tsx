...
function EditPantry() {
  ...
  async function updateCurrentPantry() {
    ...
    await supabase.from('pantries').update({ title, description }).match({ id });
  ...
  return (
    <div>
      ...
      />
      <SimpleMDE
        value={pantry?.description || undefined}
        onChange={(value) => setPantry({ ...pantry, description: value })}
      />
      <button className="mb-4 bg-blue-600 text-white font-semibold px-8 py-2 rounded-lg" onClick={updateCurrentPantry}>
        Update Pantry
      </button>
    </div>
  );
