    ...
    <div>
      {
                    ...
                    <Fragment key={category.name}>
                      ...
                      {category.products.map((product) => (
                        <tr
                          ...
                          <td className="whitespace-nowrap px-3 py-4 text-sm text-stone-500">{product.is_essential ? 'yes' : 'no'}</td>
                          <td className="whitespace-nowrap px-3 py-4 text-sm text-stone-500">{product.expires_at || 'not specified'}</td>
                          <td className="whitespace-nowrap px-3 py-4 text-sm text-stone-500">{product.vendor || ''}</td>
        ...
        ) : <EmptyState primaryAction="Add Products" onPrimaryActionClick={() => setIsProductEditorOpen(true)} header={title} subheading={description || ''}/>
      }
