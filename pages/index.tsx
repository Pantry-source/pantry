    ...
    <div>
      ...
      {pantries.map((pantry, index) => (
        <div key={index} className="border-b border-gray-300	mt-8 pb-4">
          ...
          <Link href={`/edit-pantry/${pantry.id}`}>
            <a className="text-sm mr-4 text-blue-500">Edit Pantry</a>
          </Link>
          <Link href={`/pantries/${pantry.id}`}>
            <a className="text-sm mr-4 text-blue-500">View Pantry</a>
          </Link>
          <button className="text-sm mr-4 text-red-500" onClick={() => deleteById(pantry.id)}>
            Delete Pantry
          </button>
        </div>
      ))}
